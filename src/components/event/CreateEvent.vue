      ...
      <form @submit.prevent="handleCreateEvent" class="space-y-6">
        ...
        <div>
          ...
          <textarea id="description" v-model="formData.description"
            class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-indigo-300"></textarea>
        </div>
        ...
        <!-- Submit Button -->
        <button type="submit"
          class="w-full py-3 px-6 text-white bg-indigo-600 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-300">
          Create Event
        </button>
      </form>
...
export default {
  ...
  setup() {
    ...
    const handleCreateEvent = async () => {
      try {
        await createEvent(eventStore, formData);
      } catch (error) {
